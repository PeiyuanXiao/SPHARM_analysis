...
def robust_pca_alignment(points, enforce_direction=True, verbose=False):
    """
    PCA alignment for 3D point clouds.

    Aligns the input point cloud so that the three principal axes
    correspond to X, Y, Z respectively. Enforces consistent axis
    directions across specimens by flipping axes whose median
    ...
    enforce_direction : bool
        If True, enforce consistent direction for all three principal axes.
    verbose : bool
    ...
    # Fix consistency of PC1 / PC2 / PC3 directions
    if enforce_direction:
        projected = centered @ rotation_matrix
        for axis in range(3):
            if np.median(projected[:, axis]) < 0:
...
if __name__ == "__main__":
    ...
    points_norm       = normalize_mesh(v_decim, f_decim)
    aligned, rot_mat  = robust_pca_alignment(points_norm, verbose=True)
